...
class F3Net(nn.Module):
    def __init__(self, num_classes=1, img_width=299, img_height=299, LFS_window_size=10, LFS_stride=2, LFS_M = 6, mode='Both', device=None,pretrained='imagenet'):
        ...
        self.fc = nn.Linear(3072 if self.mode == 'Both' or self.mode == 'Mix' else 1536, num_classes)
        self.dp = nn.Dropout(p=0.2)



    def init_incep_FAD(self):
        self.FAD_incep = inceptionv4(self.num_classes, self.pretrained)
        if self.pretrained == 'imagnet':
            state_dict = get_incep_state_dict()
            features_0_conv_weight=state_dict['features.0.conv.weight'].data
            self.FAD_incep.load_state_dict(state_dict,False)
            self.FAD_incep.features[0].conv=nn.Conv2d(12, 32, 3, 2, 0, bias=False)
            for i in range(4):
                self.FAD_incep.features[0].conv.weight.data[:, i*3:(i+1)*3, :, :]=features_0_conv_weight/4.0
        else:self.FAD_incep.features[0].conv=nn.Conv2d(12, 32, 3, 2, 0, bias=False)



    def init_incep_LFS(self):
        self.LFS_incep=inceptionv4(self.num_classes, self.pretrained)
        if self.pretrained=='imagnet':
            state_dict = get_incep_state_dict()
            features_0_conv_weight=state_dict['features.0.conv.weight'].data
            self.LFS_incep.load_state_dict(state_dict,False)
            self.LFS_incep.features[0].conv=nn.Conv2d(self._LFS_M, 32, 3, 1, 0, bias=False)
            for i in range(int(self._LFS_M / 3)):
                self.LFS_incep.features[0].conv.weight.data[:, i*3:(i+1)*3, :, :]=features_0_conv_weight/float(self._LFS_M / 3.0)
        else:self.LFS_incep.features[0].conv=nn.Conv2d(self._LFS_M, 32, 3, 1, 0, bias=False)
    ...
    def forward(self, x):
        if self.mode == 'FAD':
            ...
            fea_FAD = self.FAD_incep.features(fea_FAD)
        ...
        if self.mode == 'LFS':
            ...
            fea_LFS=self.LFS_incep.features(fea_LFS)
...
def get_incep_state_dict(pretrained_path='./pretrained/inceptionv4-8e4777a0.pth'):
    state_dict = torch.load(pretrained_path)
    state_dict = {k: v for k, v in state_dict.items() if 'last_linear' not in k}
    return state_dict
