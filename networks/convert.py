...
def split_high_low(img):
    radius_ratio = 0.5  # 圆形过滤器的半径：ratio * w/2
    D = 2  # 高斯过滤器的截止频率：2 5 10 20 50 ，越小越模糊信息越少
    # img = cv2.imread(r'./example.png', cv2.IMREAD_GRAYSCALE)

    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    low_freq_part_img, high_freq_part_img = get_low_high_f(img_gray, radius_ratio=radius_ratio,
                                                           D=D)  # multi channel or single
    # return  low_freq_part_img,high_freq_part_img
