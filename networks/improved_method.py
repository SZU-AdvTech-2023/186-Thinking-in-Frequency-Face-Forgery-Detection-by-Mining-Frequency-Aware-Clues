...
class FAD_Head(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat((out, amplitude), dim=1)
        out = torch.cat((out, phase), dim=1)
        return out  # [N, 14, 299, 299]
...
class F3Net_improved(nn.Module):
    def __init__(self, num_classes=1, img_width=299, img_height=299, LFS_window_size=10, LFS_stride=2, LFS_M = 6, mode='Both', device=None):
        super(F3Net_improved, self).__init__()
    ...
    def init_xcep_FAD(self):
        ...
        self.FAD_xcep.conv1 = nn.Conv2d(14, 32, 3, 2, 0, bias=False)
        # for i in range(4):
...
def get_xcep_state_dict(pretrained_path='./networks/pretrained/xception-b5690688.pth'):
    # load Xception
